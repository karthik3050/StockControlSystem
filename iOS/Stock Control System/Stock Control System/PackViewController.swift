...
class PackViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(DeviceTableViewCell2.self, forCellReuseIdentifier: "DeviceCell2") 
    ...
    func fetchDeviceIDsFromFirebase() {
        let ref = Database.database().reference().child("packedDevices")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "DeviceCell2", for: indexPath) as! DeviceTableViewCell2 // Use the correct class name
    ...
    func showDeviceDetails(InvoiceNumber: String) {
        let ref = Database.database().reference().child("packedDevices").child(InvoiceNumber)
        ref.observeSingleEvent(of: .value, with: { (snapshot) in
...
class DeviceTableViewCell2: UITableViewCell {
