...
class ShipViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(DeviceTableViewCell3.self, forCellReuseIdentifier: "DeviceCell3") 
    ...
    func fetchDeviceIDsFromFirebase() {
        let ref = Database.database().reference().child("shippedDevices")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "DeviceCell3", for: indexPath) as! DeviceTableViewCell3 // Use the correct class name
    ...
    func showDeviceDetails(InvoiceNumber: String) {
        let ref = Database.database().reference().child("shippedDevices").child(InvoiceNumber)
        ref.observeSingleEvent(of: .value, with: { (snapshot) in
...
class DeviceTableViewCell3: UITableViewCell {
